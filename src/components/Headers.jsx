    ...
    <>
      <header className="flex justify-between items-center bg-white px-[50px] w-full fixed z-10 border-b-4">
        ...
        <div className="font-bold gap-12 hidden md:flex">
          <Link
            to="/"
            className={location.pathname === '/' ? 'active' : 'text-[#CDCDCD] hover:text-[#3738dc] font-bold'}
          >
            Home
          </Link>
          <Link
            to="/articles"
            className={location.pathname === '/articles' ? 'active' : 'text-[#CDCDCD] hover:text-[#3738dc] font-bold'}
          >
            Articles
          </Link>
          <Link
            to="/categories"
            className={location.pathname === '/categories' ? 'active' : 'text-[#CDCDCD] hover:text-[#3738dc] font-bold'}
          >
            Category
          </Link>
          <Link
            to="/"
            className={location.pathname === '/about' ? 'active' : 'text-[#CDCDCD] hover:text-[#3738dc] font-bold'}
          >
            About
          </Link>
        </div>
          ...
          >
            {({ values, handleBlur, handleChange, handleSubmit, resetForm }) => (
              <form onSubmit={handleSubmit} className="bg-white flex border-2 rounded-xl px-6 items-center gap-2 h-12">
                <div>
                  <button type="submit">
                    <BsSearch size={25} color="#444cd4" className="pt-2" />
                  </button>
                </div>
                <div>
                  <input
                ...
                <div>
                  <button type="reset" onClick={() => resetForm()}>
                    <MdOutlineClear size={25} color="#444cd4" className="pt-2" />
                  </button>
                </div>
              </form>
            )}
        ...
        {token ? (
          <div className="flex justify-center items-center">
            <div className="dropdown dropdown-bottom dropdown-end">
              <label
                tabIndex={0}
                onClick={getDataNotif}
                className="btn m-1 bg-white outline-none border-0 hover:bg-white "
              >
                {' '}
                <MdNotificationsNone size={25} color="#444cd4" />
              </label>
              <ul
                tabIndex={0}
                className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-[500px] px-2 flex flex-col items-center justify-between"
              >
                            ...
                            <div className="flex gap-5 justify-start">
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex flex-col">
                                  <label htmlFor="notification" className="flex gap-2 font-bold">
                                    {author.fullName} sent you an {messageRequest}
                                  </label>
                ...
                <Link to="/notification">
                  <li className="font-bold text-primary">
                    <a className="hover:bg-white text-[#444cd4] hover:text-black">See More</a>
                  </li>
                </Link>
              </ul>
            </div>
            <div>
              <div className="dropdown dropdown-bottom dropdown-end">
                ...
                <ul
                  tabIndex={0}
                  className="dropdown-content menu p-2 shadow  bg-base-100 rounded-box w-[250px] px-2s flex flex-col items-center justify-between "
                >
                  <li>
          ...
          <div className=" items-center gap-8 font-bold hidden md:flex">
            ...
            <div>
              <Link to="/signin" className="hover:text-[#444cd4] font-bold">
                Log In
              </Link>
            </div>
          </div>
