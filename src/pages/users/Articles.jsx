    ...
    async function filterArticles(values, setArticles) {
        try {
            ...
            const { data } = await http().get(
                `/articles?limit=100&sortBy=${values.sortBy}`
            )
            setArticles(data.results)
        } catch (error) {
            const message = error?.response?.data?.message
                ...
                <div className="w-full relative">
                    ...
                    <div className="absolute top-0 left-0 w-[668px] p-20">
                        ...
                        <div className="w-[497px] text-[20px] ">
                            <p>
                                You can be an author by being active in reading
                                artciles in a month or you can request to be an
                                author if you have been a member for three
                                months.
                            </p>
                        </div>
                        <div className="absolute bottom-0 left-30">
                            <Link to="/writearticles">
                                <div className="mt-8 bg-[#444cd4] hover:bg-[#6261df] w-36 h-16 rounded-xl text-[20px] flex items-center justify-center ">
                                    <button className="btn btn-primary text-white w-full h-full">
                                        Start Writing
                                    </button>
                                </div>
                            </Link>
            ...
            <div className="bg-white px-[60px] pt-[65px] pb-[90px]">
                <div>
                    <Formik
                        ...
                        onSubmit={(values) =>
                            filterArticles(values, setArticles)
                        }
                    >
                        {({
                            values,
