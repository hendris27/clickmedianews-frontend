...
const ArticleView = () => {
    ...
    function publishButton() {
        const confirmed = window.confirm(
            'Are you sure to Publish this Articles'
        )
        if (confirmed) {
            setEdit(false)
    ...
    const getArticleData = React.useCallback(async () => {
        let data = {}
        if (token) {
            const { data: res } = await http(token).get(
                `/articles/${id}/logged`
            )
            data = res
            ...
            const { data: res } = await axios.get(
                `http://localhost:8888/articles/${id}`
            )
            data = res
        }
        setArticle(data.results)
                    ...
                    <div className="flex gap-20">
                        ...
                        <div className="flex flex-1 flex-col gap-14">
                            ...
                            <div className="flex gap-5 items-center">
                                ...
                                <button
                                    onClick={() => createSavePost(article.id)}
                                >
                                    {savePost ? (
                                        <BsBookmarkFill
                                            size={35}
